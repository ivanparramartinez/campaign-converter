<template>
  <div class="page">
    ...
    <div class="tool-header">
      <div class="logo">Interior <span>Builder</span></div>
    ...
    <div class="layout">
      <div class="sidebar">
        ...
        <div class="section">
          <div class="section-title">Interior Pairs <span class="cnt">{{ pairs.length }}</span></div>
          <div class="input-row">
            <input v-model="pairCode" placeholder="H2G" style="width:70px;flex:none" @keydown.enter="focusText" />
            <input v-model="pairText" ref="textRef" placeholder="Jet Black Vinyl" @keydown.enter="addPair" />
            <button class="btn btn-sm" @click="addPair">+</button>
          </div>
          <div class="hint">Or bulk paste (code + space/tab + text):</div>
          <div class="input-row">
            <textarea v-model="bulkInput" rows="3" placeholder="H2G	Jet Black Vinyl&#10;H1T	Jet Black Cloth" style="font-size:11px" />
            <button class="btn btn-sm" style="align-self:flex-start" @click="bulkAdd">Add</button>
          </div>
          <div class="tag-list">
            <span class="tag pair" v-for="(p,i) in pairs" :key="i">{{ p.code }} → {{ p.text }}<span class="del" @click="pairs.splice(i,1)">×</span></span>
            <button v-if="pairs.length" class="tag-clear" @click="pairs=[]">Clear</button>
          </div>
          <button v-if="g.interiorPairs.length && !pairs.length" class="btn btn-sm" style="margin-top:4px" @click="pairs=g.interiorPairs.map(p=>({code:p.code,text:p.text}))">Load from globals ({{ g.interiorPairs.length }})</button>
        </div>
...
function comboKey(obj) {
  return [obj.BRAND||'', String(obj.MODEL_YR), obj.VEH_NAME_PLATE||'', obj.INTERIOR_CODE||'', obj.INTERIOR_TEXT||''].join('|').toUpperCase()
}
...
function generate() {
  if (!brand.value || !years.value.length || !nameplates.value.length || !pairs.value.length)
    return alert('Fill in brand, year, nameplate and at least one interior pair.')
  const combos = []
  for (const y of years.value) for (const np of nameplates.value) for (const p of pairs.value) {
    const obj = { BRAND: brand.value, MODEL_YR: y, VEH_NAME_PLATE: np, INTERIOR_CODE: p.code, INTERIOR_TEXT: p.text }
...
function downloadOutput() {
  ...
  a.download = `interior-collection${suffix}.json`; a.click()
}
</script>
...
.tag.pair { border-color: rgba(192,132,252,0.3); color: #c084fc; }
